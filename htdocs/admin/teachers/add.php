...
$page_title = 'নতুন শিক্ষক যোগ';

$stmt = $conn->query("SELECT MAX(id) as max_id FROM teachers");
$row = $stmt->fetch();
$auto_id = $row['max_id'] ? intval($row['max_id']) + 1 : 1;
$auto_card_id = 'TCH' . str_pad($auto_id, 3, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($subject)) $errors[] = 'বিষয় নির্বাচন করুন';
    if (empty($designation)) $errors[] = 'পদবী নির্বাচন করুন';
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO teachers (name, number, blood_group, location, card_id, photo_path, subject, designation) 
                    VALUES (:name, :number, :blood_group, :location, :card_id, :photo_path, :subject, :designation)";
            ...
            $_SESSION['success'] = 'শিক্ষক যোগ করা হয়েছে!';
...
<div class="form-container">
    <div class="form-card">
        <h2>
            <i class="fas fa-chalkboard-teacher"></i> নতুন শিক্ষক যোগ
        </h2>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="photo-upload">
                <img id="preview" src="https://ui-avatars.com/api/?name=Teacher&background=22c55e&color=fff&size=128" 
            ...
            <div class="form-grid">
                ...
                <div>
                    <div class="form-group">
                        ...
                        <input type="text" name="number" required value="<?php echo htmlspecialchars($_POST['number'] ?? ''); ?>">
                    </div>
                </div>
                ...
                <div>
                    <div class="form-group">
                        <label>পদবী <span class="required">*</span></label>
                        <select name="designation" required>
                            <option value="">নির্বাচন করুন</option>
                            <?php foreach ($designations as $d): ?>
                                <option value="<?php echo $d; ?>" <?php echo ($_POST['designation'] ?? '') == $d ? 'selected' : ''; ?>><?php echo $d; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
